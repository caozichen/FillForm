...
function inferVariableKind(key = '', value = '') {
  ...
  if (/(银行卡|銀行卡|银行卡号|銀行卡號|bank\s*(card|account)|debit\s*card|card\s*(number|no\.?))/.test(hint) && !/(id\s*card|identity|passport|身份证|身份證|证件|證件)/.test(hint)) return 'bankCard';
  if (/(统一社会信用代码|統一社會信用代碼|social credit code|unified social credit|business registration|brn|company registration)/.test(hint)) return 'companyId';
