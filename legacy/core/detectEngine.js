...
function detectKindFromHint(field) {
  ...
  if (/(公司|企業|企业|機構|机构|政府|government|organization|organisation|company).*(名稱|名称|name)|cert.*name/i.test(hint)) return FIELD_KINDS.COMPANY_NAME;
  if (isBankCardHint(hint)) return FIELD_KINDS.BANK_CARD;
  if (/(统一社会信用代码|統一社會信用代碼|社会信用代码|社會信用代碼|信用代码|信用代碼|納税人識別號|纳税人识别号|商業登記(?:證)?號(?:碼)?|商业登记(?:证)?号(?:码)?|商業登記|商业登记|商業登記證|商业登记证|brn|business registration(?: number| no\.?| #)?|registration number|證照編號|证照编号)/i.test(hint)) return FIELD_KINDS.COMPANY_ID;
  if (/(驗證碼|验证码|verify code|verification code)/i.test(hint)) return FIELD_KINDS.VERIFICATION;
  if (/(職位|职位|職稱|position|title)/i.test(hint)) return FIELD_KINDS.JOB_TITLE;
  if (/(姓氏|姓\(中\)|姓\(英\)|last name|family name|surname)/i.test(hint)) return FIELD_KINDS.LAST_NAME;
  if (/(名字|名\(中\)|名\(英\)|first name|given name)/i.test(hint)) return FIELD_KINDS.FIRST_NAME;
  if (/邮箱|郵箱|電郵|email/i.test(hint)) return FIELD_KINDS.EMAIL;
  if (/身份证|证件|id\s*card|idcard/i.test(hint)) return FIELD_KINDS.IDCARD;
